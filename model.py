...
def model(images, weight_decay=1e-5, is_training=True):
    ...
    with tf.variable_scope('feature_fusion', value=[end_points.values]):
        batch_norm_params = {
            'decay': 0.997,
            'epsilon': 1e-5,
            'scale': True,
        ...
        with slim.arg_scope([slim.conv2d],
            ...
            def feature_merge(data, d_concat, num_output):
                ...
                return slim.conv2d(conv1x1_res, num_output, 1)
            ...
            geo_map = slim.conv2d(h[3], 4, 1,
                    activation_fn=tf.nn.sigmoid,
                    nomalizer_fn=None) * FLAG.text_scale
            angle_map = slim.conv2d(h[3], 1, 1,
                    activation_fn=tf.nn.sigmoid,
                    nomalizer_fn=None)
